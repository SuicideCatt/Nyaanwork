...
export module Nyaanwork.WindowSystem:Exception;

import Nyaanwork.Core.Types;
import Nyaanwork.Core.Exception;

export namespace Nyaanwork::WindowSystem::Error
{
	enum class Code : ErrorCodeMask
	{
		no_error = 0,

		fail_to_init_any_instance
	};
}

export template<>
struct std::is_error_code_enum<Nyaanwork::WindowSystem::Error::Code>
...
export namespace Nyaanwork::WindowSystem::Error
{
	struct Category : Nyaanwork::BaseCategory<Code>
	{
		static constexpr std::array errors = {
			...
			ErrorInfo{
				Code::fail_to_init_any_instance,
				Condition::invalid,
				"Fail to init any instance"
			},
		};

		const char* name() NYAAN_CNOEX override
			{ return "Nyaanwork::WindowSystem"; }
